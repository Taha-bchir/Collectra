...
const required = (name: string, devFallback?: string): string => {
  ...
  if (empty && devFallback !== undefined) {
    return devFallback
  }
  if (empty) {
    throw new Error(
...
export function getAuthStorageKey(): string {
  return required('NEXT_PUBLIC_APP_AUTH_STORAGE_KEY', 'saas.auth')
}
...
export function getPreferencesStorageKey(): string {
  return required('NEXT_PUBLIC_APP_PREFERENCES_STORAGE_KEY', 'saas.preferences')
}
